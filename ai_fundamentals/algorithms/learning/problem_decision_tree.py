...
def build_tree(X, y, feat_names):
    ...
    if len(np.unique(y)) == 1:
        ...
        print(f"\nNode: {node}")
        print(f"Node Prediction: {node.prediction}")
        return node

    # Check if there's any features to split on
    if X.shape[1] == 0 or len(feat_names) == 0:
        print("\nCheck if there's any feaetures to split on")
    ...
    print("\nINITIALLY:")
    # !!! --> Problem is in best_split_ig function
    print(f"Best split information gain: {best_ig}")
    print(f"Best fature name: {best_feat_name} ") # None
    print(f"Best feature index: {best_feat_idx}") # None
    ...
    print(f"\nRemaining names are: {remaining_names}")
    ...
    for val in np.unique(X[:, best_feat_idx]):
        ...
        print(f"\nAfter removing index (new X_subset): {X_subset}")
    ...
    print(f"\nFinally, the node = {node}")
    return node

def predict_one(node, x, feat_names):
    """
    Predicts class label for single example x.
    -----------------------------------------a
    INPUT:
        node: (?)
        x: (any datatype)
...
def print_tree(node, indent=""):
    ...
    # Printing feature node splits on
    print(f"{indent}[{node.feat_name}] (IG={node.info_gain:.3f})")

    # Iterate thru each branch value, getting keys/values
    for val, child in node.kids.items():
        # Branch label
...
tree = build_tree(X, y, feat_names)
print(f"Tree: {tree}")
